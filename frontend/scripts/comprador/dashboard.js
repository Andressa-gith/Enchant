...
function initializeApp(session) {
    ...
    function updateMainChart() {
        ...
        if (activeChartView === 'financeiro') {
            ...
            chartConfig = { type: 'bar', data: { labels: dataForChart.labels, datasets: [ { label: 'Entradas', data: dataForChart.datasets[0].data, backgroundColor: '#72330f', borderRadius: 5 }, { label: 'Saídas', data: dataForChart.datasets[1].data, backgroundColor: '#c2794a', borderRadius: 5 } ] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true } } } };
        } else { // estoque
