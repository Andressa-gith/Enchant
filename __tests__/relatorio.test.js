 ...
 * @version 11.0 (Mock do Supabase explícito, com cadeias isoladas e caminhos corrigidos)
 */
...
// 3. Mock do Supabase Client (Arquitetura V11 Estável)
...
// [CORREÇÃO] Renomeados para clareza
const mockGetRelatoriosFinal = jest.fn();
...
const mockGet_Order = jest.fn(() => mockGetRelatoriosFinal());
const mockGet_Eq = jest.fn(() => ({ order: mockGet_Order }));
const mockGet_Select = jest.fn(() => ({ eq: mockGet_Eq }));

// Cadeia 2: addRelatorio -> .insert().select().single()
const mockAdd_Single = jest.fn(() => mockAddRelatorioFinal());
...
const mockDelFetch_Single = jest.fn(() => mockDeleteFetchFinal());
...
const mockDelDel_Eq = jest.fn(() => mockDeleteFinal());
...
describe('Relatorio Controller', () => {
    ...
    beforeEach(() => {
        ...
        // Limpa mocks globais
        mockGetRelatoriosFinal.mockReset();
        mockAddRelatorioFinal.mockReset();
        mockDeleteFetchFinal.mockReset();
        mockDeleteFinal.mockReset();
    ...
    describe('getRelatorios', () => {
        it('deve retornar 200 e a lista de relatórios', async () => {
            ...
            mockGetRelatoriosFinal.mockResolvedValue({ data: mockData, error: null });
        ...
        it('deve retornar 500 se o Supabase falhar', async () => {
            ...
            mockGetRelatoriosFinal.mockResolvedValue({ data: null, error: mockError });
    ...
    describe('addRelatorio', () => {
        ...
        it('deve retornar 201 e adicionar o relatório com sucesso', async () => {
            ...
            mockAddRelatorioFinal.mockResolvedValue({ data: mockData, error: null });
        ...
        it('deve retornar 500 e tentar rollback se o upload do Storage falhar', async () => {
            ...
            expect(mockAddRelatorioFinal).not.toHaveBeenCalled(); // Não tentou inserir no DB
            expect(res.status).toHaveBeenCalledWith(500);
        ...
        it('deve retornar 500 e FAZER O ROLLBACK se a inserção no DB falhar', async () => {
            ...
            mockAddRelatorioFinal.mockResolvedValue({ data: null, error: mockError }); // DB Falha
            ...
            expect(mockAddRelatorioFinal).toHaveBeenCalled(); // 2. DB Falha
            expect(res.status).toHaveBeenCalledWith(500);
    ...
    describe('deleteRelatorio', () => {
        it('deve retornar 200 e deletar o relatório (DB e Storage)', async () => {
            ...
            // 1. Fetch do caminho
            mockDeleteFetchFinal.mockResolvedValue({ data: { caminho_arquivo: MOCK_FILE_PATH }, error: null });
            // 2. Delete do DB
            ...
            // Assert
            expect(mockDeleteFetchFinal).toHaveBeenCalled(); // 1. Fetch
            expect(mockDeleteFinal).toHaveBeenCalled(); // 2. Delete DB
        ...
        it('deve retornar 404 se o relatório não for encontrado', async () => {
            ...
            mockDeleteFetchFinal.mockResolvedValue({ data: null, error: null });

            // Act
            await deleteRelatorio(req, res);

            // Assert
            expect(mockDeleteFetchFinal).toHaveBeenCalled();
        ...
        it('deve retornar 500 se a exclusão do DB falhar', async () => {
            ...
            mockDeleteFinal.mockResolvedValue({ error: mockError });
